...
def refresh_all(force: bool = False):
    ...
    try:
        db = next(get_db())
        # Get titles and modified_at timestamps
        entries = db.query(WikiEntry.title, WikiEntry.modified_at).all()
        ...
        for entry_title in entries:
            try:
                should_update, _ = should_update_entry(db, entry_title.title, entry_title.modified_at)
